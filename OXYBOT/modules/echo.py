    ...
    if event.sender_id in SUDO_USERS:
        if event.reply_to_msg_id:
            ...
            if user_id in OXYGEN:
                await event.reply("ɴᴏ, ᴛʜɪꜱ 🇹 𝙾 𝚇 𝙸 𝙲 ɪꜱ ᴅᴇᴠᴇʟᴏᴘᴇʀ ᴏꜰ ᴛʜᴇꜱᴇ ʙᴏᴛꜱ🙏.")
            elif user_id == OWNER_ID:
                await event.reply("ɴᴏ,  🇹 𝙾 𝚇 𝙸 𝙲  ɪꜱ ᴏᴡɴᴇʀ ᴏꜰ ᴛʜᴇꜱᴇ ʙᴏᴛꜱ😎.")
            elif user_id in SUDO_USERS:
                await event.reply("ɴᴏ, 🇹 𝙾 𝚇 𝙸 𝙲 🇰 𝙰 🇩 𝙾 𝚂 𝚃 🇭 𝚄 🇧 𝙷 𝙾 𝚂 𝙳 𝙺🤒.")
            else:
                try:
                ...
                if check in ECHO:
                    await event.reply("» ᴇᴄʜᴏ ɪꜱ ᴀʟʀᴇᴀᴅʏ ᴀᴄᴛɪᴠᴀᴛᴇᴅ ᴏɴ ᴛʜɪꜱ ᴍᴀᴅʜᴀʀᴄʜᴏᴅ !!")
                else:
                    ECHO.append(check)
                    await event.reply("» ᴇᴄʜᴏ ᴀᴄᴛɪᴠᴀᴛᴇᴅ ᴏɴ ᴛʜᴇ ᴍᴀᴅʜᴀʀᴄʜᴏᴅ ✅")
        else:
...
async def rmecho(event):
    if event.sender_id in SUDO_USERS:
        if event.reply_to_msg_id:
            ...
            if check in ECHO:
                ECHO.remove(check)
                await event.reply("» ᴇᴄʜᴏ ʜᴀꜱ ʙᴇᴇɴ ꜱᴛᴏᴘᴘᴇᴅ ꜰᴏʀ ᴛʜᴇ ᴍᴀᴅʜᴀʀᴄʜᴏᴅ !! ☑️")
            else:
